...
func InsertPersonalia(profilePenyedia structs2.TblProfilePenyedia, userHelper helperusermodel.HelperUser) {
  ...
  if helperUser.DbPenyedia.String == "vms_db" {
    dbTable = "tbl_personalia_perusahaan"
  }
  if helperUser.DbPenyedia.String == "promise_sibela" {
    dbTable = "tbl_personalia_perusahaan1"
  }
  ...
  if helperUser.DbPenyedia.String == "promise_sibela" {
    rVTPP, errVTPP = db.PromiseSibela.Query(ctx, qVmsTblPersonaliaP, strconv.Itoa(int(profilePenyedia.IdProfilPenyedia.Int32)))
  }
	...
	for _, vTPP := range allVTPP {
		...
		//fmt.Printf("IdPersonalia: %d\n", vTPP.IdPersonalia.Int32)

		//IdPersonalia = vTPP.IdPersonalia.Int32
...
func GetKodeJenjangPendidikan(pendidikanPersonal pgtype.Text) pgtype.Int4 {
	...
	for _, helperJenjangPendidikanKw := range allHelperJenjangPendidikanKw {
		...
		str := strings.ToLower(pendidikanPersonal.String)
		substr := strings.ToLower(helperJenjangPendidikanKw.Keyword.String)
	...
	if (kodeJenjangPendidikan == pgtype.Int4{}) {
		//fmt.Printf("IdPersonalia: %d\n", IdPersonalia)
    // fmt.Printf(pendidikanPersonal.String+" --> %d\n", kodeJenjangPendidikan.Int32)
    if pendidikanPersonal.String != "-" {
  		fmt.Printf(pendidikanPersonal.String + "\n")
    }
	}
