...
func TruncateTable(tblName string) {
	...
	_, err := db.DbSiplang.Exec(context.Background(), qTruncate)
	if err != nil {
...
func AlterAllForeignKey(action string) {
	...
	if action == "create" {
		path = filepath.Join("files", "siplang", "sql", "foreign_key_create.sql")
	} else if action == "drop" {
		path = filepath.Join("files", "siplang", "sql", "foreign_key_drop.sql")
	} else {
	...
	_, errQFk := db.DbSiplang.Exec(ctx, qFk)
	if errQFk != nil {
		//fmt.Println("qFk: " + qFk)
...
func UpdatePkSequence(tableName string, pkFieldname string) {
	...
	_, errUpdateSeq := db.DbSiplang.Exec(ctx, qUpdateSeq)
	if errUpdateSeq != nil {
...
func UpdatePrimaryKeySequenceFromFile() {
	...
	path = filepath.Join("files", "siplang", "sql", "update_serial_sequence.sql")
