...
func InsertNew(refPermintaan RefPermintaan) RefPermintaan {
  ...
  if refPermintaan.JenisPenyedia.String == "luardpt" {
	...
	if errIns != nil {
		log.Fatal("unable to insert ref_permintaan siplang, " + errIns.Error())
	}
	...
	if errRwIns != nil {
		fmt.Printf("refPermintaan.KodeSkemaPembayaran: %d\n", refPermintaan.KodeSkemaPembayaran.Int32)
		log.Fatal("failed collecting RefPermintaan siplang (ref_permintaan.go), " + errRwIns.Error())
	}
...
func GetAllData() []RefPermintaan {
	...
	rwRefPermintaan, errQuery := db.DbSiplang.Query(ctx, qAllData)
	if errQuery != nil {
...
func DeleteByKodePermintaan(kodePermintaan pgtype.Int4) {
	...
	_, err := db.DbSiplang.Exec(ctx, qDelete, kodePermintaan)
	if err != nil {
...
func DeleteWhereUcrNull() {
	...
	_, err := db.DbSiplang.Exec(ctx, qDelete)
	if err != nil {
...
func UpdateHps(kodePermintaan pgtype.Int4, nilaiHps pgtype.Int4) {
	...
	_, err := db.DbSiplang.Exec(ctx, qUpdate, nilaiHps, kodePermintaan)
	if err != nil {
...
func UpdateKodeStatusPermintaan(kodePermintaan pgtype.Int4, kodeStatusPermintaan pgtype.Int4) {
	...
	_, err := db.DbSiplang.Exec(ctx, qUpdate, kodeStatusPermintaan, kodePermintaan)
	if err != nil {
