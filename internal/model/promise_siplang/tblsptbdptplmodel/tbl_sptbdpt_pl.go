...
func GetByIdTerminPl(idTerminPl pgtype.Int4) structs.TblSptbPl {
	...
	SELECT
		...
		dpp_ppn,
		dpp_pph,
		ppn_dptsptb AS ppn_sptb,
		pph_dptsptb AS pph_sptb,
		jenis_pph,
		id_objek_pajak,
		additional,
		dok_non_pkp,
		...
		pph_persen,
		transaksi,
		tertanggal_dptsptb AS tertanggal_sptb
	FROM tbl_sptbdpt_pl
	WHERE id_termin_dptpl = $1`

	rwSptbPl, err := db.PromiseSiplang.Query(ctx, qSptbPl, idTerminPl)
	if err != nil {
